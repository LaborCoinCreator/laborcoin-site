(() => {
  ...
  function requireActive() {
    if (!isActive()) {
      throw new Error(
        "LaborCoin Revision 7 is in predeployment mode. Contract interactions are disabled until all seven final addresses and runtime commitments are verified."
      );
    }
    return config;
  }
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    banner.textContent =
      "Revision 7 predeployment source candidate: interactions are disabled until the seven contracts are compiled, deployed, and runtime-verified.";
